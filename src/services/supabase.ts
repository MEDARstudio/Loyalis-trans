  ...
  async getVouchers(): Promise<Voucher[] | null> {
    ...
    try {
      ...
      return data.map(row => ({
        ...
        isValidated: Boolean(row.is_validated) || String(row.created_by_agent || '').toLowerCase().includes('amine'),
        validatedBy: row.validated_by || (String(row.created_by_agent || '').toLowerCase().includes('amine') ? 'Amine' : undefined),
        validatedAt: row.validated_at || (String(row.created_by_agent || '').toLowerCase().includes('amine') ? row.created_at : undefined),
        validationNotes: row.validation_notes || (String(row.created_by_agent || '').toLowerCase().includes('amine') ? 'Validé automatiquement (Créé par l\'administrateur)' : ''),
  ...
  async insertOrUpdateVoucher(v: Voucher): Promise<boolean> {
    ...
    try {
      const row = {
        ...
        is_validated: v.isValidated !== undefined ? (Boolean(v.isValidated) || String(v.createdByAgent || '').toLowerCase().includes('amine')) : String(v.createdByAgent || '').toLowerCase().includes('amine'),
        validated_by: v.validatedBy || (String(v.createdByAgent || '').toLowerCase().includes('amine') ? 'Amine' : null),
        validated_at: v.validatedAt || (String(v.createdByAgent || '').toLowerCase().includes('amine') ? (v.createdAt || new Date().toISOString()) : null),
        validation_notes: v.validationNotes || (String(v.createdByAgent || '').toLowerCase().includes('amine') ? 'Validé automatiquement (Créé par l\'administrateur)' : ''),
